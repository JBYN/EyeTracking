...
# @return:
def getFace(parameters: c.Eye_Parameters) -> c.Face:
    ...
    if faces != ():
        ...
        for (x, y, w, h) in faces:
            face1 = dlib.rectangle(left=int(x), top=int(y), right=int(x + w), bottom=int(y + h))
            p_face = c.Rectangle(x, y, w, h)
            roi_gray = gray[p_face.y:p_face.y + p_face.height,
                       p_face.x:p_face.x + p_face.width]  # pixels of the region of interest
            roi_color = parameters.image[p_face.y:p_face.y + p_face.height, p_face.x:p_face.x + p_face.width]

            eyeCorner = ec.EyeCorner(parameters.image, face1)
            cv2.circle(parameters.image, (eyeCorner.getPosLeftEyeCorner().x, eyeCorner.getPosLeftEyeCorner().y), 1,
                       (0, 0, 255))
            cv2.circle(parameters.image, (eyeCorner.getPosRightEyeCorner().x, eyeCorner.getPosRightEyeCorner().y), 1,
                       (0, 0, 255))
            # preprocessing
            ...
            if p_eyes != None:
                ...
                if p_checkedEyes != None:
                    ...
                    img_leftEye = roi_gray[p_leftEye.y:p_leftEye.y + p_leftEye.height,
                                  p_leftEye.x:p_leftEye.x + p_leftEye.width]
                    img_rightEye = roi_gray[p_rightEye.y:p_rightEye.y + p_rightEye.height,
                                   p_rightEye.x:p_rightEye.x + p_rightEye.width]
                    ...
                    eye_stroke = 2
                    # indicator
                    ind_leftEye = cv2.rectangle(roi_color, (p_leftEye.x, p_leftEye.y),
                                                (p_leftEye.x + p_leftEye.width, p_leftEye.y + p_leftEye.width),
                                                eye_color_L, eye_stroke)
                    ind_rightEye = cv2.rectangle(roi_color, (p_rightEye.x, p_rightEye.y),
                                                 (p_rightEye.x + p_rightEye.width, p_rightEye.y + p_rightEye.width),
                                                 eye_color_R, eye_stroke)
                    return face
            else:
...
def get_positionPupil(eye: np.ndarray, weightBlurSize: int) -> c.Point:
    ...
    _, threshold = cv2.threshold(blur_Eye, 45, 255, cv2.THRESH_BINARY_INV)
...
def scalePosition(face: c.Face, pos_left: c.Point, pos_right: c.Point) -> c.EyePupils:
    leftPupil_scaled = c.Point((pos_left.x + face.leftEye.pEye.x + face.pFace.x),
                               (pos_left.y + face.leftEye.pEye.y + face.pFace.y))
    rightPupil_scaled = c.Point((pos_right.x + face.rightEye.pEye.x + face.pFace.x),
                                (pos_right.y + face.rightEye.pEye.y + face.pFace.y))
